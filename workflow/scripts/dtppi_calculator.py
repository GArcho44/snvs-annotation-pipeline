...
def merge_overlapping_clusters(cluster_sets, min_overlap=5):
    merged = True
    while merged:
        merged = False
        out = []
        while cluster_sets:
            first = cluster_sets.pop()
            overlap_found = False
            for i, second in enumerate(cluster_sets):
                if len(first & second) >= min_overlap:
                    cluster_sets[i] = first | second
                    overlap_found = True
                    merged = True
                    break
...
def group_interface_residues(pdb_file, pred_df, protein_id):
    ...
    if len(coords) < 6:
        # Nothing to cluster if fewer than 6 residues
        return set(), [], {}
    ...
    for cluster in cluster_map.values():
        ...
        res_ids_cur = [c[0] for c in cluster]
        avg_plddt = np.mean([
            structure[0][rid[0]][(" ", rid[1], " ")]['CA'].get_bfactor()
            for rid in res_ids_cur
        ])
        if (avg_conf > 0.8) and (avg_plddt > 70) and (len(cluster) >= 6):
            candidate_clusters.append(set(res_ids_cur))

    merged_clusters = merge_overlapping_clusters(candidate_clusters.copy())
    ...
    residue_to_cluster = {}
    for new_id, res_set in enumerate(merged_clusters, start=1):
        confs = [pred_lookup[r] for r in res_set]
        plddts = [structure[0][rid[0]][(" ", rid[1], " ")]['CA'].get_bfactor()
                  for rid in res_set]
        summary = {
            "protein_id": protein_id,
            "cluster_id": new_id,
            "num_residues": len(res_set),
            "avg_confidence": np.mean(confs),
            "avg_pLDDT": np.mean(plddts),
            "residues": ";".join([f"{c}:{r}" for c, r in sorted(res_set)])
        }
        cluster_summaries.append(summary)
        high_quality_residues.update(res_set)

        # map every residue to its cluster for later join
        for rid in res_set:
            residue_to_cluster[rid] = new_id


    return high_quality_residues, cluster_summaries, residue_to_cluster


def calculate_interface_distance(protein_id, pdb_path, interface_residues, residue_to_cluster):
    all_coords, all_residues = extract_structure_coords(pdb_path)

    # keep the ordering of interface IDs alongside their coordinates
    interface_coords = []
    ...
    for res_id, coord in zip(all_residues, all_coords):
        if res_id in interface_residues:
            ...
            interface_cluster_ids.append(residue_to_cluster[res_id])
    ...
    raw_distances = []
    for res_id, coord in zip(all_residues, all_coords):
        cluster_id = residue_to_cluster.get(res_id, np.nan)


        if res_id in interface_residues:
            dist = 0.0
            cluster_id = residue_to_cluster[res_id]
        elif len(interface_coords) > 0:
            dists = np.linalg.norm(interface_coords - coord, axis=1)
            idx = int(np.argmin(dists))
            dist = dists[idx]
            cluster_id = interface_cluster_ids[idx]
            raw_distances.append((res_id, dist))
        else:
            # no interface residues in this protein
            cluster_id = np.nan
            continue

        if raw_distances:                               # at least one non-interface residue
            max_dist = max(d for _, d in raw_distances)
        else:                                           # every residue was an interface
            max_dist = 1.0                              # arbitrary non-zero to avoid /0
        norm_dist = dist / max_dist

        dist_list.append((protein_id, res_id[0], res_id[1], cluster_id, dist, norm_dist))

    if dist_list:
        df = pd.DataFrame(dist_list, columns=["protein_id", "chain_id", "residue_id", "cluster_id" , "DTPPI", "normalized_DTPPI"])
        return df
    else:
        return None
...
def batch_interface_distance(structure_dir, pesto_csv_path, output_dir, combined_output_file):
    ...
    if all_data:
        ...
        combined_residue_df = pd.DataFrame()
    ...
    if not combined_residue_df.empty or not cluster_summary_df.empty:
        merged_df = combined_residue_df.merge(
            cluster_summary_df,
    ...
    if not cluster_summary_df.empty:
        cluster_summary_file = os.path.join(
            os.path.dirname(combined_output_file),
            "interface_clusters_summary.csv")
